...
//**************************************************************************************************
void dspfuncs();
void tftlog(int section, const char *str);
...
// Global variables
// TSQueue<tftMessage> lcdtextqueue;
...
String icyurl;                                // Icecast station name
...
bool dspftrigger = false;                     // To trigger execution of special functions
...
void queueToPt(qdata_type func)
{
  ...
  while (xQueueReceive(dataqueue, &specchunk, 0))
    ;                                            // Empty the queue
  ...
  if (inx < TFTSECS) // Segment available on display
  {
    if (str) // String specified?
    {
      if (strlen(str) > 0)
      {
        ...
        message.section = inx;
        xQueueSend(dspqueue, &message, 0);
        // myQueueSend(dspqueue, &message);
        //  lcdtextqueue.push(message);
        //   tftdata[inx].str = String(str); // Yes, set string
        //   ESP_LOGI("tftset", "setting tft %d %s", inx, str);
        //   tftdata[inx].pos = 0;
        //   tftdata[inx].update_req = true; // and request flag
        //   ESP_LOGI(TAG, "Section %d update req", inx);
      }
    }
  }
}
...
//     ESP_LOGI("tftset", "setting tft %d %s",inx, str.c_str());
//     tftdata[inx].pos = 0;
//     tftdata[inx].update_req = true; // and request flag
//     //ESP_LOGI(TAG, "Section %d update req", inx);
//   }
...
void cleanStreamtitle(String &streamtitle)
{
  // remove quotes
  if (streamtitle[0] == '\'')
    streamtitle[0] = ' ';
  if (streamtitle[streamtitle.length() - 1] == '\'')
    streamtitle[streamtitle.length() - 1] = ' ';
  streamtitle.trim();
  // contains StreamURL remove it
  int nottitle = streamtitle.indexOf(";");
  if (nottitle > 0)
  {
    streamtitle = streamtitle.substring(0, nottitle);
  }
}
//**************************************************************************************************
...
bool showstreamtitle(const char *ml, bool full)
{
  char *p1;
  char *p2;
  static String oldstreamtitle; // Previous title, for compare
  String streamtitle(ml);       // Streamtitle from metadata

  if (!full)
  {
    if (streamtitle.startsWith("StreamTitle="))
    {
      streamtitle = streamtitle.substring(12, streamtitle.length() - 1);
      cleanStreamtitle(streamtitle);
      if (streamtitle.length() > 0)
      {
        ESP_LOGI(TAG, "Streamtitle found, %d bytes", streamtitle.length());
        // ESP_LOGI(TAG, "%s", streamtitle.c_str());
    ...
    //               streamtitle) != 0)
    //{
    tftset(1, streamtitle.c_str()); // Yes, set screen segment text middle part
...
bool connecttohost()
{
  ...
  String s("Connecting... ");
  s += presetinfo.host;
  tftset(0, s.c_str());                 // Set screen segment text top line
  tftset(1, " ");                       //  tftset(1, "");                        // Clear song and artist
...
bool connectwifi()
{
  ...
  else
  {
    String s("SSID = ");
    s += WiFi.SSID();
    tftlog(0, s.c_str());      // Show SSID on display
...
void otaerror(ota_error_t error)
{
  ...
  tftset(1, "OTA error!"); // Set screen segment bottom part
}
#endif // ENABLEOTA
...
void setup()
{
  ...
  dspqueue = xQueueCreate(200,               // Create queue for data communication
                          sizeof(tftMessage));
  log_printf("\n");
  ...
  dataqueue = xQueueCreate(QSIZ, // Create queue for data communication
                           sizeof(qdata_struct));
  tftset(0, "Connecting newtwork...");
  dspftrigger = true;
  dspfuncs();
  p = "Connect to network"; // Show progress
  ESP_LOGI(TAG, "%s", p);
  tftlog(0, p); // On TFT too
#ifdef ETHERNET
...
void chk_enc()
{
  ...
  if (doubleclick) // Handle the doubleclick
  {
    ...
    tftset(1, "Turn to select station\nPress to confirm");
  ...
  if (singleclick)
  {
    ...
    case VOLUME:
      if (muteflag)
      {
        tftset(0, icyname.c_str()); // Restore screen segment bottom part
      }
      else
      {
        tftset(1, "Mute");
    ...
    case TRACK:
      ...
      enc_menu_mode = VOLUME;          // Back to default mode
      tftset(0, icyname);              // Restore screen segment bottom part
      break;
#endif
    default:
      break;
  ...
  case PRESET:
    ...
    tftset(1, tmp.c_str()); // Set screen segment bottom part
    break;
#ifdef SDCARD
  case TRACK:
    if (rotationcount > 0)
    {
      ...
      tftset(1, getCurrentShortSDFileName()); // Set screen segment bottom part
    }
    break;
#endif
  default:
...
void spfuncs()
{
  if (spftrigger) // Will be set every 100 msec
  {
    ...
    if (dsp_ok)         // Posible to update TFT?
      // {
      //   tftMessage nextMessage = lcdtextqueue.pop();
      //   if (nextMessage.section != -1)
      //   {
      //     ESP_LOGI(TAG, "New message for section %d : %s", nextMessage.section, nextMessage.message.c_str());
      //     tftdata[nextMessage.section].refreshCount = 100;
      //     tftdata[nextMessage.section].str = nextMessage.message;
      //     tftdata[nextMessage.section].pos = 0;
      //     dsp_update(enc_menu_mode == VOLUME, nextMessage.section); // Updates to the screen
      //   }
      //   for (uint16_t i = 0; i < TFTSECS; i++) // Yes, handle all sections
      //   {
      //     //update all the others
      //     if (tftdata[i].scrollable && i != nextMessage.section) // Refresh requested?
      //     {
      //       //ESP_LOGI(TAG, "Section %d updating", i);
      //       dsp_update(enc_menu_mode == VOLUME, i); // Updates to the screen
      //     }
      //   }
      // }
    ...
    if (time_req) // Time to refresh timetxt?
    {
      ...
      tftset(2, timetxt);                              // Write to TFT screen
...
void dspfuncs()
{
  if (dspftrigger) // Will be set every 100 msec
  {
    dspftrigger = false; // Reset trigger
    if (dsp_ok)          // Posible to update TFT?
    {
...
void handlebyte_ch(uint8_t b)
{
  ...
  if (datamode == INIT) // Initialize for header receive
  {
    ...
    metalinebf[0] = '\0';
    // showstreamtitle("StreamTitle=No title...");
  }
    ...
    else if (b == '\n') // Linefeed ?
    {
      ...
      if (chkhdrline(metalinebf))     // Reasonable input?
      {
        ...
        else if (lcml.startsWith("icy-name:"))
        {
          ...
          tftset(0, icyname.c_str());    // Set screen segment bottom part
      ...
      if (LFcount == 2) // Double LF marks end of header?
      {
        if (redirection) // Redirection?
        {
          ...
          tftset(1, "More than 5 redirection, stopped");
          redirectioncount = 0;
        }
...
void displayinfo(uint16_t inx)
{
  ...
  if (dsp_ok) // TFT active?
  {
    ...
    if (len++)             // Check string length, set buffer length
    {
      ...
      dsp_setCursor(0, p->row);     // Prepare to show the info
      // String b(buf);
      // dsp_println(b); // Show the string
    }
  }
}
